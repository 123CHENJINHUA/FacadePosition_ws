...
def _create_sam3_transformer(has_presence_token: bool = True, image_size=1008) -> TransformerWrapper:
...
def _load_checkpoint(model, checkpoint_path):
    ...
    # Filter out freqs_cis buffers that have size mismatch due to resolution change
    keys_to_remove = []
    for k, v in sam3_image_ckpt.items():
        if "freqs_cis" in k:
            # Check if shape matches
            # We can't easily check against model state dict here without iterating
            # But since we know these are precomputed buffers dependent on resolution,
            # it is safe to skip loading them if we changed resolution.
            # They are re-computed in __init__ of the model.
...
def build_sam3_image_model(
    ...
    vision_encoder = _create_vision_backbone(
        compile_mode=compile_mode, enable_inst_interactivity=enable_inst_interactivity, image_size=image_size
    )
    ...
    # Create transformer
    transformer = _create_sam3_transformer(has_presence_token=True, image_size=image_size)
